...
public class TermBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public List<Term> getTerms(){
        ...
        ServiceProvider providerA = ServiceProvider.builder().id(1l).name("providerA").email("AP").password(passwordEncoder.encode("passAP")).userRole(PROVIDER.getUserRole()).address("ul.Osobowicka 5").city("Wrocław").build();
        ServiceProvider providerB = ServiceProvider.builder().id(2l).name("providerB").email("BP").password(passwordEncoder.encode("passBP")).userRole(PROVIDER.getUserRole()).address("ul.Czekoladowa 1").city("Warszawa").build();
        ...
        Term term2 = Term.builder().id(2l).termDate(LocalDate.parse("2021-11-19")).termTime(LocalTime.parse("13:32:22", DateTimeFormatter.ISO_TIME)).service(service2).client(clientA).termRealizedStatus(Status.FULL).build();
        Term term3 = Term.builder().id(3l).termDate(LocalDate.parse("2021-11-20")).termTime(LocalTime.parse("14:32:22", DateTimeFormatter.ISO_TIME)).service(service1).client(clientA).termRealizedStatus(Status.FULL).build();
        Term term4 = Term.builder().id(4l).termDate(LocalDate.parse("2021-11-19")).termTime(LocalTime.parse("09:32:22", DateTimeFormatter.ISO_TIME)).service(service1).client(clientA).termRealizedStatus(Status.FULL).build();
        Term term5 = Term.builder().id(5l).termDate(LocalDate.parse("2022-11-22")).termTime(LocalTime.parse("09:32:22", DateTimeFormatter.ISO_TIME)).service(service2).termRealizedStatus(Status.FREE).build();
        Term term6 = Term.builder().id(6l).termDate(LocalDate.parse("2022-11-15")).termTime(LocalTime.parse("09:32:22", DateTimeFormatter.ISO_TIME)).service(service1).termRealizedStatus(Status.FREE).build();
        Term term7 = Term.builder().id(7l).termDate(LocalDate.parse("2022-11-23")).termTime(LocalTime.parse("09:32:22", DateTimeFormatter.ISO_TIME)).service(service1).termRealizedStatus(Status.FREE).build();
